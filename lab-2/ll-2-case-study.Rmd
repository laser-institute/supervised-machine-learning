...
### Quickly calculating metric

After all of this work, we *can* calculate the above much more easily
given how we setup our metrics (using `metric_set()` earlier. Below, use
`collect_metrics()` with the `final_fit` object, noting that in addition
to the four metrics we calculated manually just a few moments ago, we
are also provided with the accuracy and Cohen's Kappa values.
