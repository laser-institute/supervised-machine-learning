...
::: {.notes}
You can lean into other ideaes - writing other ideas (e.g., do moon phases matter for certain RQs!) on the whiteboard.
:::
...
**In this lab, we focus on using [OULAD data](https://analyse.kmi.open.ac.uk/open_dataset) on interactions data, the most fine-grained data**
...
In module 1, we fit and interpreted a single _fit of our model_ (80% training, 20% testing). What if we decided to _add new features_ or _change existing features_?
...
- Boosted trees, like those created using the xgboost package, sequentially build trees where each new tree attempts to correct the errors made by the previous ones, effectively reducing bias and improving accuracy.

- By focusing on correcting mistakes and optimizing the model iteratively, boosted trees can achieve better performance compared to random forests, which average the predictions of many uncorrelated trees, potentially reducing variance but not necessarily bias.

- Boosted trees offer more hyperparameters to fine-tune, such as learning rate and tree depth, providing greater flexibility and potential for optimization, often leading to superior performance on complex datasets.
