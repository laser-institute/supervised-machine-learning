...
Next, we'll load a new file --- one with *interactions* (or log-trace) data. Please find that (`oulad-interactions.csv`) in the `data` folder for this lab, read that in and assign it the name `interactions`. 

*N.b.* **If you download these materials directly from GitHub, you will need to unzip the file titled `oulad-interactions.csv.zip`**
...
Then, let's engineer several features. For the present time, we'll focus on the `sum_click` variable, which tells us how many times students clicked on a resource for a given date. We'll use the `interactions` data set we just created. Let's start simple: please type the name of that data set (`interactions`) in the code chunk below. Note, you may need to unzip this file!

#### **👉 Your Turn** **⤵**

```{r}
interactions_filtered <- read_csv("data/oulad-interactions.csv")
```
