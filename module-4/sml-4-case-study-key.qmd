...
Please load `oulad-interactions-filtered.csv` into R, assigning the resulting
data frame the name `interactions`. Note, you may need to unzip this file!
...
choosing. Options include creating simple graphs or calculating
...
This is so we have the cut-offs
...
`interactions_summarized` and `interactions_slopes`, assigning the
resulting output the name `interactions_summaried_and_slopes`.
...
`students_assessments_and_interactions`. Lots of joining! Sometimes, the hardest part of complex analyses lies in the preparation (and joining) of the data.
...
folds from 10 to 5 by changing the value of v; 10 is simply the
...
variables---features---we created.  We've started you off with ___ code below (where you can add the new variables -- be sure to add all of them!).
...
the `mean_weighted_score`, `sum_clicks`, `sd_clicks`, `mean_clicks`, `intercept`, `slope`, and `date_registration` variables.
