...
First, though, we need to load *a really big file* with all of the interactions data. In the OULAD documentation, this is called the VLE (virtual learning environment) data source. Please review the description of the variables in the *studentVLE* and *VLE* sources (which are joined together for this module) [here](https://analyse.kmi.open.ac.uk/open_dataset#description). Then, read in the `oulad-interactions.parquet` file. Instead of `read_csv()`, you'll need to use a function from the {arrow} package, `read_parquet()`:
