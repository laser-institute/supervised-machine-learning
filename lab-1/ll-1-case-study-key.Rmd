...
several others focused on modeling: tidymodels and janitor. As a tip, use the  
`library()` function to load these packages. After you've done that, like in earlier learning labs, click
...
It is in the `data` sub-folder and is named `ngsschat-processes-data.csv`.

Load it and assign the name `d` to it in the code chunk below, then run the chunk.
...
Go ahead and type `data_train` and `d` below to
...
can safely ignore that in this instance.
...
Nice work - you've completed this case study! Move on
