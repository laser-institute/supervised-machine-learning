...
In the chunk below, examine the data set using a function or means of your choice (such as just *printing* the data set by typing its name or using the `head()` function). Do this in the code chunk below! Note its dimensions --- especially how many rows it has!
...
ipeds_df = ipeds_df.dropna()  # Todo(josh and aaron): discuss dropping vs imputation
train_df, test_df = train_test_split(ipeds_df, test_size=0.2, random_state=42)  # splitting the data and sedding a random state so this is reproducible.
...
X_train, X_test = train_df[independent_cols], test_df[independent_cols]  # split our dataset between dependent and independent columns
...
model.fit(X=X_train, y=y_train)  # Train the model using the training dataset

y_preds = model.predict(X_test)  # Make predictions on the test dataset
accuracy = accuracy_score(y_test, y_preds)  # use sklearn built in functions to 
