...
Let's create a recipe where we predict `pass` (the outcome variable) on the basis of the `disability` and `imd_band` (predictor) variables. Add these two variables to your recipe, below, replacing \_\_\_ below with that code.

```{r}
my_rec <- recipe(pass ~ ___, data = data_train)
...
my_mod <-
    logistic_reg() %>% 
    set_engine("XXX") %>% # generalized linear model
    set_mode("YYY") # since we are predicting a dichotomous outcome, specify classification; for a number, specify regression
...
Add your model and recipe (see their names above), again. replacing the placeholders below.

```{r}
my_wf <-
    workflow() %>% # create a workflow
    add_model(MODEL) %>% # add the model we wrote above
    add_recipe(RECIPE) # add our recipe we wrote above
```
