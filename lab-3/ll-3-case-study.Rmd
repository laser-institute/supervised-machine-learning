...
Woah, this is a large file! Let's start to explore it a bit. Please take
three steps to explore the data, as follows.
...
`code_module_dates` data frames. This is so we have the cut-offs
(one-third through the semester), which will help us to only use data
from that period in our model when we filter the data based upon those
dates.
...
clickstream data! What are some other features we can add? An affordance
...
Please mutate the resulting intercept, date, and data squared variables
